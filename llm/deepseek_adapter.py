...
class DeepSeekAdapter(BaseLLM):
    ...
    def generate(self, prompt: str, **kwargs: Any) -> str:
        ...
        if not self.api_key:
            raise RuntimeError("DeepSeekAdapter requires `api_key`.")
        if not self.base_url:
            raise RuntimeError("DeepSeekAdapter requires `base_url`.")
        if not self.model:
            raise RuntimeError("DeepSeekAdapter requires `model`.")
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if exc.code in {429, 500, 502, 503, 504} and attempt < self.max_retries:
                    ...
                    continue
                raise RuntimeError(f"DeepSeek API HTTPError {exc.code}: {details}") from exc
            ...
            if last_exc is not None:
                raise RuntimeError(f"DeepSeek API connection error: {last_exc}") from last_exc
            raise RuntimeError("DeepSeek API request failed without response.")
        ...
        if not isinstance(content, str):
            raise RuntimeError(f"DeepSeek API returned invalid content: {data}")
        return content
