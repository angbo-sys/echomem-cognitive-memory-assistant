...
本项目是“中文信息处理”课程设计作品，围绕中文学习对话、中文记忆检索、用户画像演化和可解释记忆框架集成展开。
...
EchoMem 当前已经具备完整原型能力，但仍然是课程设计阶段作品，还存在一些不足：

- **回复体验还不够实时**：LLM 回复目前仍是完整返回，还不是逐 token 流式输出，长回答时需要等待完整生成。
- **UI 自动化验收不足**：界面已经过人工和基础组件测试，但尚未接入桌面/移动截图级视觉回归，颜色、字号、重叠等问题仍需要人工复查。
- **编排逻辑偏集中**：`Orchestrator` 承担了较多流程控制职责，后续可以拆成更清晰的 Pipeline，降低维护和测试成本。
- **配置治理还可继续细化**：API 配置已经统一，但 UI 尺寸、Prompt 权重、预算阈值等产品参数还可以进一步集中管理。
